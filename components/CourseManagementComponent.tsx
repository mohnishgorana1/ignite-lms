...
function CourseManagementComponent({ course }): any {
  ...
  return (
    <section>
      <header className="w-full grid grid-cols-2 gap-y-2 sm:grid-cols-4 sm:px-10 gap-x-3 sm:gap-x-5">
        <Button
          onClick={() => setIsViewCourseDetailDialogOpen(true)}
          className="px-2 sm:px-4 py-1 h-8 sm:h-9 border-transparent bg-blue-700 text-white hover:bg-blue-800"
        >
          View Course Details
        </Button>
        <Button
